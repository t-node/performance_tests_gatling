...
/**
  ...
  */
object TheBayAddToBag extends BaseTest{
  ...
  val TheBayAddToBag = group("TheBayAddToBag_Transaction") {
      exec(http("AjaxOrderChangeServiceItemAdd")
        ...
        .exec(session => {
       //   val url2 = baseUrlHttp + "/webapp/wcs/stores/servlet/en/thebay/AjaxPromoGiftItemView"
          val url3 = baseUrlHttp + "/webapp/wcs/stores/servlet/en/thebay/AjaxQuickCartDisplay?storeId=${StoreID}&catalogId=${catalogID}&langId=-24"
          val url4 = baseUrlHttp + "/webapp/wcs/stores/servlet/PromotionFreeGiftChoicesView?catalogId=${catalogID}&langId=-24&storeId=${StoreID}"
      ...
      .get(baseUrlHttp + "/webapp/wcs/stores/servlet/ShopCartProcessCmd?catalogId=${catalogID}&orderId=.&langId=-24&storeId=${StoreID}")

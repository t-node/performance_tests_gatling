...
/**
  ...
  */
object SaksO5ChangeQty extends BaseTest{
//  val random = new util.Random
//  val feeder = Iterator.continually(Map("email" -> (random.nextString(20) + "@foo.com")))

  val chkUrl = baseUrlHttps + "/checkout/checkout.jsp"
  val ChangeQty_Transaction = instance + "ChangeQty_Transaction"
  val update_cart_item_quantity_service = "update_cart_item_quantity_service"
  val ChangeQty = group(ChangeQty_Transaction) {
    exec(session => {
      ...
      .exec(session => {
        ...
        insertGroupResponseTime(session("startTime").as[Long],EndTime,instance + "SaksO5_ChangeQty")
        session

...
import SaksChains.SaksCheckoutChains.SaksHomePage._
  ...
  val AddToBag = group(AddToBag_Transaction) {
    exec(session => {
      ...
      .exec(session => {
        ...
        val checkout3 = baseUrlHttp + "/include/saksbaginsert.jsp?PRODUCT%3C%3Eprd_id=" + prd_id + "&FOLDER%3C%3Efolder_id=" + folder_id + "&bmUID=" + bmUID

        //  println(session.attributes)
        session.set("checkoutUrl", checkout2).set("checkoutUrl2",checkout3).set("referer",checkouts)
      ...
      .exec(http(add_saks_suggests_item_service_product_array)
        ...
        .check(regex("""displayAddToBag":true""").count.saveAs("AddToBag_valid"))
           .check(bodyString.saveAs("JSONItem"))

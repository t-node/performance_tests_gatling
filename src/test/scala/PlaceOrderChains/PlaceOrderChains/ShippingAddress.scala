...
import SaksO5Chains.SaksO5CheckoutChains.SaksO5SearchStringPA._
...
/**
  ...
  */
object ShippingAddress extends BaseTest{
//  val random = new util.Random
//  val feeder = Iterator.continually(Map("email" -> (random.nextString(20) + "@foo.com")))
val ShippingAddress_Transaction = instance + "ShippingAddress_Transaction"
  val validate_ship_address_service ="validate_ship_address_service"
  val add_address_and_continue_service = "add_address_and_continue_service"
  val creditCardTypes = "bmForm=get_option_list_service&listName=creditCardTypes"

  val chkUrl = baseUrlHttps + "/checkout/checkout.jsp"

  val Shipping = group(ShippingAddress_Transaction) {
    feed(ShippingDetailsPO)
      .exec(session => {
     ...
     //   .check(bodyString.saveAs("ShippingAddress1"))
      ...
      .exec(session => {
        ...
        insertGroupResponseTime(session("startTime").as[Long],EndTime,instance + "SaksO5_ShippingAddress")
        session
